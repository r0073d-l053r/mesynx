...
- **WhisperX service hardened.** `BATCH_SIZE` default lowered from 16 to 8 with measured per-VRAM guidance (16 OOMs on an 8 GB card; use 4 there), `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True` to stop fragmentation creep on long-lived containers, English-only alignment preload by default via `WHISPERX_ALIGN_LANGUAGES` (~4.1 GB → ~0.4 GB resident; other languages still load on demand), and an entrypoint guard that idles with a clear message instead of crash-looping when `HF_TOKEN` is unset. The `whisper` service and the `gpu` profile gating are unchanged, so CPU-only installs are unaffected.

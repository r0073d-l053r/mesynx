...
- **WhisperX is now the only transcription service.** The bundled [faster-whisper-server](https://github.com/fedirz/faster-whisper-server) `whisper` service has been removed from `docker-compose.yml` — WhisperX supersedes it (same transcription, plus forced alignment and diarization) and the two competed for the same GPU. WhisperX is no longer behind a compose profile, so a plain `docker compose up -d` starts it. **If your default transcription provider still points at `http://whisper:8000/v1`, transcription will fail until you repoint it** — see [Upgrading](content/docs/self-hosting/upgrading.mdx).
...
- **The bundled `whisper` service** (`fedirz/faster-whisper-server`) and its `whisper_cache` volume declaration. Superseded by WhisperX. The volume itself is left intact so no downloaded models are destroyed on upgrade; reclaim it with `docker volume rm <project>_whisper_cache` once you're satisfied the new stack works.

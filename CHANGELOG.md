...
> Mesynx AI is a distribution of [Riffado](https://github.com/riffado/riffado). The entries below are the
> changes layered on top of the upstream Riffado base.

### Added

- **Speaker names, resolved everywhere from one rename.** Diarized transcripts now carry editable speaker labels. Click any speaker badge — in the transcript, the summary, or the workspace — type a name, and it propagates to every surface at once: transcript, summary prose, key points, action items, node labels, section tags, breadcrumbs and graph. Stored in `transcriptions.speaker_names` (migration `0026`, encrypted) and written by `PATCH /api/recordings/[id]/speakers`. Raw `SPEAKER_NN` ids are what get persisted; names are resolved at render time, which is what makes a rename retroactive *and* repeatable. The map is cleared on re-transcription, because a fresh diarization run reassigns ids arbitrarily.
- **Editable transcription workspace.** Replaces the read-only Memory Map with three linked views over one editable node tree — a hand-arranged **Mind Map**, an Obsidian-style **Graph**, and an indented **Document** outline — with full CRUD on nodes, summaries, time ranges, tags, colours and transcript chunks. Arrangements persist to `transcriptions.workspace_nodes` (migration `0027`, encrypted) via a debounced `PATCH /api/recordings/[id]/workspace`, which re-validates the whole tree server-side including an explicit parent-cycle walk. A **Rebuild** button discards a saved layout so a recording can pick up improvements to the derivation.
- **Obsidian-style graph view.** Force-directed canvas graph with no external graph library. Solves to rest before first paint and then freezes, so nothing drifts unless you drag it; dragging holds the simulation warm and releasing pins the node where you dropped it. Node radius encodes subtree size rather than link degree, so the root reads as the largest node instead of the smallest.
- **Transcript evidence under every section.** Sections now carry the transcript that supports them, grouped by which child owns each run, so drilling from a summary claim down to the words that produced it works at every level. Pure keyword matching plateaued at 34% coverage on real audio (most lines are "yeah, exactly"); an anchor-and-fill pass — high-confidence lexical matches become anchors, everything else joins the nearest anchor by position — reaches 100% in 25 contiguous runs.
- **`patches/`.** Unified diffs for the third-party transcription images, with `patches/apply.sh` to materialise them. See [`patches/README.md`](patches/README.md).
...
- **WhisperX is now the only transcription service.** The bundled [faster-whisper-server](https://github.com/fedirz/faster-whisper-server) `whisper` service has been removed from `docker-compose.yml` — WhisperX does everything it did plus forced alignment and diarization, and running both meant two model sets competing for one card. WhisperX is also no longer behind the `gpu` profile, so a plain `docker compose up -d` starts it. **BREAKING for any install whose transcription provider points at `http://whisper:8000/v1`** — the stack still comes up healthy and the provider row still looks fine, so the first sign is a failed transcription. See [Upgrading](content/docs/self-hosting/upgrading.mdx).
- **`scripts/install.sh` detects the NVIDIA runtime.** Since WhisperX carries a GPU device reservation, `docker compose up` fails outright on a host without the NVIDIA container runtime. The installer now checks first and, if it is missing, brings up `db` and `app` only with a clear message about what to install, instead of dying halfway through. It also fetches `patches/whisperx-audio.py.patch` + `apply.sh` and materialises the patch, without which diarization 404s. The now-dead `docker-compose.gpu.yml` overrides (which only upgraded the removed `whisper` service to CUDA) are gone, and the release workflow publishes the patch files in their place.
- **WhisperX service hardened.** `BATCH_SIZE` default 4 with measured per-VRAM guidance, `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True` to stop fragmentation creep on long-lived containers, English-only alignment preload via `WHISPERX_ALIGN_LANGUAGES` (~4.1 GB → ~0.4 GB resident; other languages still load on demand), an entrypoint guard that idles with a clear message instead of crash-looping when `HF_TOKEN` is unset, and the GPU reservation pinned to device 0 so a resident LLM on another card cannot contend for it.
- **Summary and title token budgets raised** to 8000 and 800 (from 2000 and 50). Reasoning models emit reasoning tokens against the same `max_tokens` budget before producing any content, so the old caps could be consumed entirely by reasoning. Non-reasoning models stop at EOS well below either ceiling, so the headroom costs them nothing.
- **Speaker names are no longer baked into generated summaries.** The transcript is sent to the model with raw `SPEAKER_NN` ids intact and names are substituted at display time. Previously a name was frozen into the generated prose at write time, so a later rename could never reach an existing summary.
...
- **The bundled `whisper` service** (`fedirz/faster-whisper-server`) and its `whisper_cache` volume declaration. Superseded by WhisperX. The volume itself is deliberately left on disk so no downloaded models are destroyed by an upgrade; reclaim it with `docker volume rm <project>_whisper_cache` once the new stack is working. Note this makes GPU-capable hardware a requirement for transcription — there is no CPU image for WhisperX.
- **`MemoryMap`.** Replaced by the editable transcription workspace. It derived a throwaway tree from summary sentences on every render, with no model, no editing and no persistence.
...
- **Transcription containers permanently unhealthy, and restart-looping under any supervisor.** Both the `whisper` and `whisperx` healthchecks invoked `curl`, which **neither image ships** — so the check could never pass, the container reported unhealthy from the moment it was created, and anything that restarts on unhealthy (autoheal and similar) restarted it forever. Seen in the wild as **5,126 restarts over 43 days**, which silently killed every transcription that outlived the restart window and presented as "Whisper is inaccurate". Both checks now use `python3`, which both images do have; whisperx additionally probes `/health` rather than `/v1/models`, since the latter requires the bearer key. If you run a restart-on-unhealthy supervisor, this is the entry to care about.
- **`-diarize` model names returning 404.** Mesynx requests diarization by appending `-diarize` to the model name, but the stock WhisperX image rejects any model name that isn't exactly `TRANSCRIBE_MODEL`, so the documented convention never worked against it. Fixed by a one-line bind-mounted patch; run `./patches/apply.sh` once before starting whisperx. See [`patches/README.md`](patches/README.md).
- **Hour-long recordings transcribing as one phrase repeated hundreds of times.** The long-standing "Whisper is inaccurate" bug, and it was never the model. Three causes compounded: quiet wearable audio (a clip-on recorder averages around −30 dB with long silences), `vad_filter` defaulting **off** so that silence reached the decoder, and — the decisive one — the app correctly sending `condition_on_previous_text: false` while the stock server **silently discarded unknown form fields**, so faster-whisper's default `True` applied and one bad 30-second window fed itself as context forever. Measured on the same file: 2,668 → **7,756 words**, duplicate segments 99.9% → 5.2%, distinct vocabulary ~3 → **1,200**. Fix shipped as [`patches/stt.py.patch`](patches/stt.py.patch).
- **Summaries returning blank or truncated with no error.** `finish_reason` was not inspected anywhere in the codebase, so a response cut off at `max_tokens` was persisted as the summary and returned HTTP 200 — empty content fell through the JSON parse fallback as `""`, and a truncated fragment was stored verbatim. Both now raise `AI_PROVIDER_API_ERROR` (502) naming the model. Summaries generated before this fix may be blank or fragmentary; regenerate to repair one.
- **`null:` prefixes throughout diarized transcripts.** Roughly 5% of segments come back unattributed (filler, crosstalk) and the formatter emitted `${seg.speaker}: ` unconditionally. Worse, unattributed segments are often *first*, and `isDiarized()` only inspected line one — so an entire diarized transcript could fall back to plain-paragraph rendering with `null:` visible throughout. Unattributed segments now emit bare text, parsed as a continuation of the previous speaker, and `isDiarized()` scans the first 12 non-empty lines.
- **The speaker rename editor closing itself.** The same speaker renders a badge in the legend *and* beside every run, so clicking one mounted two `autoFocus` inputs; the second stole focus, which blurred and committed the first. The editor is now keyed to the specific badge instance clicked.
- **React #418 hydration mismatch on the dashboard.** Two time-dependent renders disagreed between server and client: elapsed-time drift, and — the worse one — `isToday`/`isYesterday` evaluated against a **UTC** server and a local-time browser, so a recording near midnight rendered `TODAY` server-side and `YESTERDAY` client-side. Fixed with a `useHydrated()` hook **plus** `suppressHydrationWarning`; both halves are required, since `suppressHydrationWarning` alone silences the warning while leaving the server's wrong text on screen.
- **WhisperX exhausting GPU memory after long uptime.** Resident memory crept to 6.5 GB of an 8 GB card over ~17 hours and then failed mid-transcription with `CUDA failed with error out of memory`, surfacing to the app as a bare 500 with nothing in its own logs. A restart reclaimed it to 2.6 GB, so most of the growth was allocator fragmentation rather than the model. Mitigated with `BATCH_SIZE=4` and `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`; peak on a 17-minute diarized run is now ~4.4 GB.
...
### Fixed
- Plaud sync from hosted/VPS deploys whose ASN is flagged by Cloudflare: pair the Webshare residential proxy from v0.5.1 with a Chrome TLS/JA3 fingerprint via `wreq-js`, so Bun's default handshake stops getting a 403 + Cloudflare challenge even from a clean residential IP. Required because Cloudflare scores ASN and TLS fingerprint independently; #148 only addressed the ASN side. Direct path (no `WEBSHARE_API_KEY` set) is unchanged and does not load the new dependency at runtime ([#152](https://github.com/r0073d-l053r/mesynx/pull/152)).

## [0.5.1] - 2026-05-15

### Added
- Optional outbound proxy for Plaud API calls via Webshare residential proxies. When `WEBSHARE_API_KEY` is set, every request to `api*.plaud.ai` / `resource.plaud.ai` routes through a random valid proxy from the Webshare list, with automatic rotation on Cloudflare 403/407 responses. Unset (default) keeps the direct egress path. Required for hosted deployments on flagged datacenter ASNs where Cloudflare returns a 403 HTML challenge before requests reach Plaud's origin; not needed for residential / homelab self-hosts. No effect on non-Plaud outbound traffic. `scripts/plaud-egress-probe.sh` can validate proxy behavior end-to-end against a real account ([#148](https://github.com/r0073d-l053r/mesynx/pull/148)).

## [0.5.0] - 2026-05-15

### Added
- Automation API: versioned `/api/v1/recordings` namespace for external agents (list with cursor pagination + `created_since` / `updated_since` / `has_transcription` filters, get, transcript, metadata, audio redirect). Personal API keys (`op_...` prefix, HMAC-SHA256 hashed at rest keyed by `API_TOKEN_HASH_SECRET ?? BETTER_AUTH_SECRET`, revocable, with `source` / `name` / `lastUsedAt` metadata) managed from Settings. Per-IP and per-identity rate limits on `/api/v1/*` (1200/min IP, 600/min identity) with `Retry-After` and `X-RateLimit-*` headers. Signed webhooks (HMAC-SHA256, `t=...,v1=...` header) for `recording.synced`, `recording.updated`, `recording.deleted`, `transcription.completed`, `transcription.failed` with URL + secret encrypted at rest, exponential backoff (30s -> 6h, then dead), DB-leased worker, manual redelivery, and a deliveries inspector. Webhook target policy switches on `WEBHOOKS_REQUIRE_PUBLIC_TARGETS ?? IS_HOSTED`: self-host defaults permissive so docker-bridge URLs like `http://n8n:5678/webhook` keep working. Full reference in `docs/API.md` ([#118](https://github.com/r0073d-l053r/mesynx/pull/118)).
- Documentation site at `/docs` with Guides, Self Hosting, and Reference sections (fumadocs-mdx). `/llms.txt` and `/llms-full.txt` routes for LLM-friendly access ([#131](https://github.com/r0073d-l053r/mesynx/pull/131)).
- Dashboard waveform player: client-side AudioContext decode on first listen, peaks cached server-side in `recordings.waveform_peaks`, canvas renderer with hover timestamp and vertical playhead. Auto-decode capped at 30min with a manual "Generate waveform" button above that. New `user_settings.player_scrubber` preference (`waveform` | `slider`) with toggle in Playback settings ([#121](https://github.com/r0073d-l053r/mesynx/pull/121)).
- Command palette (Cmd/Ctrl+K) for sync, upload, settings, theme, and recording typeahead with fuzzy search over filenames and transcript text. Inline `Transcribe` quick-action on audio-only rows; Cmd+Enter triggers transcribe on the highlighted row without closing the palette. Keyboard shortcuts cheatsheet (`?`), `j`/`k` list nav, `/` for search focus, `,` for settings ([#121](https://github.com/r0073d-l053r/mesynx/pull/121)).
- Recording list redesign: search, sort + density toggles, date-grouped rows with sticky headers, status chips (transcript / summary / in-flight), per-row delete via hover menu, infinite-scroll sentinel replacing Prev/Next, optimistic upload and delete. New `user_settings.list_density` preference ([#121](https://github.com/r0073d-l053r/mesynx/pull/121)).
- Settings redesign with grouped sidebar nav (Providers / Plaud / Personalize / Data / Integrations / Advanced), new primitives (`SettingsSectionHeader`, `SettingsCard`, `ToggleRow`), hash routing, and a master-detail mobile layout. New Storage section with usage hero, per-status breakdown bar, and top-5 largest-recordings list with deep links ([#121](https://github.com/r0073d-l053r/mesynx/pull/121)).
- Running version shown in the footer as a link to its release notes. Self-host instances also see an "update available" badge when a newer GitHub release exists (cached 5 minutes, fails silently). New optional `DISABLE_UPDATE_CHECK` env var for instances that block outbound HTTPS or want zero phone-home. `/api/health` response gains an additive `version` field ([#124](https://github.com/r0073d-l053r/mesynx/pull/124), [#123](https://github.com/r0073d-l053r/mesynx/issues/123)).
- Footer "Report a bug" button and toast `[Report]` action that pre-fill a GitHub issue with an `errorId` correlation token (and an "Email us" option on hosted). Every response with HTTP status >= 500 now carries `details.errorId` (`err_xxxxxxxx`) tying the JSON envelope to a single `console.error` log line, so users can quote it and support can grep logs. Connect / verify / paste-token error sites in `plaud-connect-tabs` migrated to `toastApiError` so the Report action lights up on 5xx ([#143](https://github.com/r0073d-l053r/mesynx/pull/143)).
- `/install` page with copyable curl one-liner and version-pinned form, plus a "View raw script" escape hatch -- replaces the previous footer link that dumped raw shell into the browser. Reachable from the footer in both self-host and hosted ([#125](https://github.com/r0073d-l053r/mesynx/pull/125)).

### Changed
- API key format adopts base62 + CRC32 checksum (`op_<base62-secret><crc32>`). New keys are 1 character shorter, checksum-validated before any DB lookup, so malformed input is rejected early. Existing keys continue to work unchanged ([#139](https://github.com/r0073d-l053r/mesynx/pull/139)).
- Sticky page header with consolidated user menu (email-initial avatar, inline 3-button theme control, identity block, kbd hints). Status-aware sync button replaces the separate status block + button -- label cycles `Synced 2m ago` / `Sync device` / `Syncing...` / `Retry sync` with tooltip carrying next-auto-sync ETA or error message ([#121](https://github.com/r0073d-l053r/mesynx/pull/121)).
- Shared imperative confirm dialog (`useConfirm()`) replaces ad-hoc destructive-action confirmations across recording delete, webhook delete, custom-prompt delete, and provider delete. One dialog UI for every destructive action ([#121](https://github.com/r0073d-l053r/mesynx/pull/121)).
- React 19 cleanup pass: `forwardRef` -> ref-as-prop, `useContext` -> `use()`, em dashes replaced with commas/colons/periods in user-facing copy, display headings tightened to `font-semibold`, hydration mismatch on locale-formatted timestamps fixed via a new `<LocalTime>` component, plus component splits (workstation, settings dialog, providers section, webhooks section, command palette, recording player, onboarding) to drop giant-component sizes. SEO metadata added for `/` and `/suspended` ([#138](https://github.com/r0073d-l053r/mesynx/pull/138)).
- Theme system rewritten as a thin wrapper over `next-themes` to fix Radix `useId` hydration drift on Next 16. `middleware.ts` renamed to `proxy.ts` for the Next 16 entrypoint change. Behavior unchanged ([#121](https://github.com/r0073d-l053r/mesynx/pull/121)).
- Release pipeline hardened against tag/version drift: `scripts/release.ts` creates both the release commit and the `[Unreleased]` cycle commit, then pushes `main` and the tag atomically; the release workflow now hard-gates on `package.json` version matching the pushed tag and checks out the tagged ref ([#124](https://github.com/r0073d-l053r/mesynx/pull/124), [#123](https://github.com/r0073d-l053r/mesynx/issues/123)).
- Footer split: minimal in-app footer (logo, AGPL, version + update badge, GitHub, hosted-only Support) for signed-in screens, and a separate hosted-only `LandingFooter` (Product / Resources / Company / Legal columns + honesty rail disclaiming HIPAA / SOC 2 certification) for marketing surfaces. Self-host always sees the minimal footer ([#125](https://github.com/r0073d-l053r/mesynx/pull/125)).

### Fixed
- Install `git` in the Docker builder so fumadocs-mdx's `lastModified` plugin can read commit timestamps; without it the docs build fell back to mtime and emitted noisy warnings ([#140](https://github.com/r0073d-l053r/mesynx/pull/140)).
- Send browser `User-Agent` on all Plaud API requests. Plaud's edge started 403-ing non-browser UAs, breaking sync on certain regional servers ([#136](https://github.com/r0073d-l053r/mesynx/pull/136), [#132](https://github.com/r0073d-l053r/mesynx/issues/132)).
- Run fumadocs-mdx codegen in the Docker builder stage instead of via `postinstall`, so generated MDX bindings ship inside the runtime image and `/docs` doesn't 500 on cold containers ([#134](https://github.com/r0073d-l053r/mesynx/pull/134)).
- Send `chunking_strategy` for diarization-capable transcription models, and consolidate the manual-transcribe and sync-transcribe code paths so they agree on request shape. Fixes diarize-model failures on long recordings ([#128](https://github.com/r0073d-l053r/mesynx/pull/128), [#101](https://github.com/r0073d-l053r/mesynx/issues/101)).
- OpenRouter transcription now routes through chat-completions (OpenRouter does not expose `/audio/transcriptions`), and the provider preset ships a curated list of transcription-capable models so the picker doesn't surface chat-only models that would 404 at request time ([#126](https://github.com/r0073d-l053r/mesynx/pull/126), [#122](https://github.com/r0073d-l053r/mesynx/issues/122)).
- Player time label no longer resizes mid-playback. `formatTimeLike(current, reference)` pads `currentTime` to match `duration`'s segment structure (`M:SS` / `MM:SS` / `H:MM:SS`) so the clock label keeps a stable width ([#121](https://github.com/r0073d-l053r/mesynx/pull/121)).
- Webhook worker no longer skews due-delivery comparisons under Bun: `nowParam` is cast to `::timestamp` to match the `next_attempt_at` column type ([#121](https://github.com/r0073d-l053r/mesynx/pull/121)).
- DELETE recording locks the parent row with `SELECT ... FOR UPDATE` at the start of the transaction, so a concurrent transcribe or summary write can't leave orphan child rows pointing at a tombstoned recording. Sync re-checks the tombstone under `FOR UPDATE` before its update + emit, preventing delete-during-download races from resurrecting a recording ([#118](https://github.com/r0073d-l053r/mesynx/pull/118)).
- Plaud helpers (`plaudSendCode`, `plaudVerifyOtp`, `listPlaudWorkspaces`, `mintPlaudWorkspaceToken`) parse upstream JSON defensively via a new `safeParseJson`. Cloudflare WAF HTML challenge pages (and other non-JSON upstream bodies) previously escaped as raw `SyntaxError` and flattened to a generic `INTERNAL_ERROR (500)`; they now surface as typed `PLAUD_INVALID_TOKEN` / `PLAUD_RATE_LIMITED` / `PLAUD_UPSTREAM_ERROR` / `PLAUD_API_ERROR` with the upstream HTTP status and a 200-char body snippet in `details` ([#143](https://github.com/r0073d-l053r/mesynx/pull/143), [#142](https://github.com/r0073d-l053r/mesynx/issues/142), [#137](https://github.com/r0073d-l053r/mesynx/issues/137)).

## [0.4.2] - 2026-05-09

### Fixed
- `/admin/reauth` returned a permanent 404 in production Docker images even with `IS_HOSTED=true` and `ADMIN_EMAILS` configured, leaving admins unable to elevate their session. The page performed its `env.IS_HOSTED` / `env.ADMIN_EMAILS` `notFound()` checks before any dynamic API call, so `next build` (which doesn't see admin env by design) statically prerendered it as a 404 and baked that into the image. Every other admin page already declared `export const dynamic = "force-dynamic"`; reauth was the only miss. Added the directive ([#113](https://github.com/r0073d-l053r/mesynx/pull/113)).
...
### Added
- Switch and disconnect Plaud account from Settings — new "Plaud Account" section shows the connected email + region, with **Switch account** (clears the connection and reopens the connect dialog) and **Disconnect** actions. Legacy connections without a stored email render as "Connected (email unknown)". Recordings are unaffected by either action ([#63](https://github.com/r0073d-l053r/mesynx/issues/63)).
...
- One-line self-host installer: `curl -fsSL https://mesynx.r0073dl053r.com/install.sh | sh`. Detects OS, verifies Docker + Compose v2, downloads `docker-compose.yml` and `env.example` from the matching GitHub release, generates `BETTER_AUTH_SECRET` / `ENCRYPTION_KEY` / `POSTGRES_PASSWORD`, starts the stack, and waits on `/api/health`. Version-pinned form available at `https://mesynx.r0073dl053r.com/vX.Y.Z/install.sh`. Source: [`scripts/install.sh`](scripts/install.sh) ([#95](https://github.com/r0073d-l053r/mesynx/issues/95)).
...
### Added
- New connect screen with three methods: **Sign in with Plaud** (browser-extension bridge — the easy path), **Email code** (existing OTP flow), and **Paste token** (advanced fallback). Targets accounts created via Google or Apple sign-in on Plaud, where the OTP flow silently signs users into a separate empty shadow account and sync returns zero recordings ([#65](https://github.com/r0073d-l053r/mesynx/issues/65)).
- Companion browser extension [r0073d-l053r/mesynx · chrome-extension](https://github.com/r0073d-l053r/mesynx/tree/main/chrome-extension) (AGPL-3.0) detected by the connect screen via `window.__mesynxAiConnector`. Lets users sign in to Plaud the way they normally do — Google, Apple, or email/password — with no copy-pasting. The extension hands the resulting access token back to Mesynx AI via the new `/api/plaud/auth/connect-token` endpoint, which encrypts it (AES-256-GCM) and persists alongside any existing OTP-flow connections.
- AI output language selector — choose the language for AI-generated summaries and titles, independent of the transcript's language ([#57](https://github.com/r0073d-l053r/mesynx/issues/57)).
- Forgot/reset password flow with email-delivered reset links via better-auth. Login surfaces the "Forgot password?" link only when SMTP is configured; reset revokes all other sessions to limit damage from compromised credentials ([#82](https://github.com/r0073d-l053r/mesynx/issues/82)).
- Delete recording action in the workstation UI ([#56](https://github.com/r0073d-l053r/mesynx/issues/56)).
- `IS_HOSTED` env flag — set to `true` on the Mesynx AI-operated hosted instance to render the marketing landing page at `/`. Defaults to `false` so self-host instances no longer serve Mesynx AI's hosted-tier marketing surface ([#70](https://github.com/r0073d-l053r/mesynx/issues/70)).
- `DISABLE_REGISTRATION` env flag — set to `true` to close a self-host instance to new sign-ups. Wired through better-auth's `disableSignUp`, the `/register` page, and the login footer link. Defaults to `false`, preserving current behavior ([#59](https://github.com/r0073d-l053r/mesynx/issues/59)).

### Changed
- Logged-out visitors at `/` now redirect to `/login` instead of seeing the marketing landing page. This is the new default for self-host. Operators who want to keep the marketing surface (or who want to host a fork's own landing page) can set `IS_HOSTED=true` ([#70](https://github.com/r0073d-l053r/mesynx/issues/70)).
- Audio duration is now parsed in JavaScript on upload instead of shelling out to `ffprobe`. The `ffprobe` binary is no longer required in the Docker image or on the host ([#58](https://github.com/r0073d-l053r/mesynx/issues/58)).

### Fixed
- Plaud recording endpoints now mint a workspace-scoped token, fixing 403s on EU/APAC accounts where the OTP-flow access token lacks workspace permissions ([#66](https://github.com/r0073d-l053r/mesynx/issues/66)).
- Settings now shows the instance storage type from the environment instead of a hardcoded value ([#78](https://github.com/r0073d-l053r/mesynx/pull/78) by [@sauerhosen](https://github.com/sauerhosen)).
...
- `MESYNX_AI_VERSION` env var for pinning the image tag.
...
[unreleased]: https://github.com/r0073d-l053r/mesynx/compare/v0.4.1...HEAD
[0.4.1]: https://github.com/r0073d-l053r/mesynx/releases/tag/v0.4.1
[0.4.0]: https://github.com/r0073d-l053r/mesynx/releases/tag/v0.4.0
[0.3.0]: https://github.com/r0073d-l053r/mesynx/releases/tag/v0.3.0
[0.2.0]: https://github.com/r0073d-l053r/mesynx/releases/tag/v0.2.0
[0.1.0]: https://github.com/r0073d-l053r/mesynx/releases/tag/v0.1.0

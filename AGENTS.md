...
Understand what you're changing. Mesynx AI is live and in use — people depend on it to access their recordings, transcripts, and storage. If you can't explain what your change does and how it interacts with the rest of the system, don't ship it.
...
Mesynx AI ships in **two production modes from the same codebase**:
...
- **The one-line installer is part of the deploy surface.** `scripts/install.sh`, the `/install.sh` route, and the version-pinned `/vX.Y.Z/install.sh` route are a single contract — the script is served from the repo file via `src/app/install.sh/route.ts` and `src/app/[version]/install.sh/route.ts`. Breaking changes (renaming, removing prompts, changing the URL shape) need a deprecation cycle and a CHANGELOG note. Self-hosters paste these URLs into their own runbooks; don't break them silently.
...
Mesynx AI is AGPL-3.0, an open-source companion app for AI voice recorders. Currently supports the Plaud Note family (Note, Note Pro, NotePin); more device support is planned.

### Positioning rules (marketing-facing)

Mesynx AI is **not** positioned as a Plaud competitor or a "subscription killer." Even though Plaud is the only device family supported today, marketing copy stays vendor-agnostic and forward-looking:

- Frame Mesynx AI as a **multi-device transcription layer**, not a Plaud alternative.
...
- A future Mesynx AI-branded device is on the roadmap but **must not be announced** in any user-facing copy until product confirms.
...
- **No vendor lock-in inside Mesynx AI either.** Storage pluggable (local / S3-compatible). AI providers pluggable (any OpenAI-compatible). Don't hardcode to one; don't add a "default cloud" fallback that silently leaks data.
...
- **Never claim compliance we don't own.** HIPAA, SOC2, attorney-client privilege — the claim belongs to the user's AI provider + their self-host setup, not to Mesynx AI. Marketing and product copy both stay honest here.
...
Mesynx AI has "marketing ahead of code" in some places. **Always cross-check landing / pricing / changelog claims against actual code before designing features that depend on them.** Known gaps:
...
- **No:** generic self-host transcription stacks (Whisper + a script). Mesynx AI must feel like a product, not a pile of utilities.
...
Mesynx AI doesn't have a per-provider abstraction — it uses the OpenAI SDK with a custom `baseURL`. "Adding a provider" is usually configuration, not code:

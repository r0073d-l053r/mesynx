...
function usage(): never {
    console.error("Usage:");
    console.error(
        "  bun scripts/release.ts <major|minor|patch|x.y.z>   # phase 1: open PR",
    );
    console.error(
        "  bun scripts/release.ts finalize                    # phase 2: tag after merge",
    );
    process.exit(1);
}
...
function bumpVersion(target: string): string {
    ...
    if (BUMP_TYPES.has(target)) {
        ...
        if (compareVersions(target, current) <= 0) {
            console.error(
                `Error: ${target} must be greater than current ${current}.`,
            );
            process.exit(1);
        }
        run(`npm version ${target} --no-git-tag-version`);
...
function updateChangelogForRelease(version: string): void {
    ...
    writeFileSync(
        "CHANGELOG.md",
        content.replace("## [Unreleased]", `## [${version}] - ${date}`),
    );
}
...
function assertCleanOnMainInSync(): void {
    ...
    if (branch !== "main") {
        console.error(
            `Error: must release from main, currently on '${branch}'.`,
        );
        process.exit(1);
    }
    ...
    if (local !== remote) {
        ...
        console.error(
            "Run `git pull --rebase` (or reconcile manually) and retry.",
        );
        process.exit(1);
    }
}
...
function phase1(target: string): void {
    console.log("\n=== Mesynx AI Release — Phase 1 (open PR) ===\n");
    assertGhAvailable();
    ...
    if (tryRun(`git rev-parse --verify ${branch}`, { silent: true }) !== null) {
        console.error(
            `Error: local branch '${branch}' already exists. Delete it or finish the prior release.`,
        );
        process.exit(1);
    }
    ...
    if (remoteBranch !== null) {
        console.error(
            `Error: remote branch '${branch}' already exists on origin.`,
        );
        process.exit(1);
    }
    const localTag = tryRun(`git rev-parse --verify refs/tags/v${version}`, {
        silent: true,
    });
    if (localTag !== null) {
    ...
    const tmpDir = mkdtempSync(join(tmpdir(), "mesynx-ai-release-"));
    ...
    console.log(
        "  2. Merge the PR (merge commit preferred; squash works too).",
    );
    console.log("  3. Run: bun scripts/release.ts finalize");
}

function phase2Finalize(): void {
    console.log("\n=== Mesynx AI Release — Phase 2 (finalize) ===\n");
    ...
    if (localTag !== null) {
        console.error(
            `Error: tag v${version} already exists locally. Aborting to avoid double-tagging.`,
        );
        process.exit(1);
    }
    const remoteTag = tryRun(
    ...
    if (remoteTag !== null) {
        console.error(
            `Error: tag v${version} already exists on origin. Aborting.`,
        );
        process.exit(1);
    }
    ...
    if (matches.length === 0) {
        console.error(
            `Error: no commit on origin/main matches 'chore(release): v${version}'.`,
        );
    ...
    if (matches.length > 1) {
        console.error(
            `Error: multiple commits on origin/main match 'chore(release): v${version}':`,
        );
        for (const sha of matches) console.error(`  ${sha}`);
        console.error(
            "Resolve manually with `git tag v" +
                version +
                " <sha> && git push origin v" +
                version +
                "`.",
        );
        process.exit(1);

...
function renderConnected(status) {
    ...
    if (status.apiBase) {
        rows.push({
            key: "API",
            value: status.apiBase.replace("https://", ""),
        });
    }
    if (status.expiresAt) {
...
function renderError(msg) {
    ...
    $("status-detail").innerHTML =
        `<p style="font-size:11px;color:#ef4444">${esc(msg)}</p>`;
}
...
async function doConnect(btn) {
    ...
    try {
        ...
        if (response?.ok) {
            showResult(
                "success",
                "Plaud account connected to Mesynx AI. The page will reload.",
            );

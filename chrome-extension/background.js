...
async function handleConnectToMesynx(mesynxUrl, sendResponse) {
    ...
    if (!token) {
        sendResponse({
            ok: false,
            error: "No Plaud session found. Sign in to web.plaud.ai first.",
        });
        return;
    }
    ...
    try {
        ...
        sendResponse({
            ok: false,
            error: `Could not open Mesynx AI: ${err.message}`,
        });
        return;
    }
    ...
    try {
        ...
        sendResponse({
            ok: false,
            error: "Mesynx AI tab took too long to load.",
        });
        return;
    }
...
async function handleStartAuth(sendResponse) {
    ...
    try {
        ...
        cleanupAuth();
        sendResponse({
            error: `Could not open the Plaud sign-in window: ${err.message}`,
        });
        return;
    }
...
chrome.tabs.onRemoved.addListener((tabId) => {
    ...
    cleanupAuth();
    respond({
        error: "The sign-in window was closed before completing. Please try again.",
    });
});
...
async function registerDynamicScriptsForUrl(url) {
    ...
    try {
        ...
        if (
            origin.includes("localhost:8790") ||
            origin.includes("mesynx.r0073dl053r.com")
        ) {
            return;
        }

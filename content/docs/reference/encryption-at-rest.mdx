...
Mesynx AI encrypts user content fields in the database with AES-256-GCM,
...
| `transcriptions.speaker_names` | jsonb | Real names you assign to diarized speakers. `{ "c": "v1:..." }` envelope |
| `transcriptions.workspace_nodes` | jsonb | Saved workspace tree — labels and section summaries are user-authored prose. Same envelope |
...
on the same machine as your Mesynx AI instance. The server still holds
